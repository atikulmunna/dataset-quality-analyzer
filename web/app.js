(() => {
  ...
  function renderRow(job, compact = false) {
    ...
    if (compact) {
      return `<tr data-status="${job.status}" data-search="${escapeHtml(`${job.name} ${job.id}`.toLowerCase())}"><td data-label="Dataset">${dataset}</td><td data-label="Job ID"><span class="job-id">${escapeHtml(job.id)}</span></td><td data-label="Preset">${escapeHtml(job.preset)}</td><td data-label="Findings"><span class="finding-count">${job.findings}</span></td><td data-label="Status"><span class="status ${job.status}">${job.status}</span></td><td data-label="Updated">${escapeHtml(job.updated)}</td><td class="row-action-cell"><button class="row-action" type="button" data-run-action="${escapeHtml(job.id)}" aria-label="Open ${escapeHtml(job.name)}">•••</button></td></tr>`;
    }
    return `<tr><td data-label="Dataset">${dataset}</td><td data-label="Preset">${escapeHtml(job.preset)}</td><td data-label="Images">${escapeHtml(job.images)}</td><td data-label="Findings"><span class="finding-count">${job.findings}</span></td><td data-label="Status"><span class="status ${job.status}">${job.status}</span></td><td data-label="Completed">${escapeHtml(job.updated)}</td><td class="row-action-cell"><button class="row-action" type="button" data-run-action="${escapeHtml(job.id)}" aria-label="Open ${escapeHtml(job.name)}">→</button></td></tr>`;
  }
